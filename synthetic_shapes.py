# credit to Remi Paturat and Paul Satlin This file is a modified version of the code
...
def generate_background(size=(960, 1280), nb_blobs=100, min_rad_ratio=0.01,
    ...
    for i in range(nb_blobs):
        col = _get_random_color(background_color)
...
def generate_custom_background(size, background_color, nb_blobs=3000,
    ...
    """
    img = np.zeros(size, dtype=np.uint8)
    img = img + _get_random_color(background_color)
...
def add_final_blur(img, kernel_size=(5, 5)):
...
def _keep_points_inside(points, size):
...
def draw_lines(img, nb_lines=10):
    ...
    for i in range(num_lines):
        ...
        if _intersect(segments[:, 0:2], segments[:, 2:4], p1, p2, 2):
            continue
...
def draw_polygon(img, max_sides=8):
    ...
    corner_angles = [_angle_between_vectors(points[(i-1) % num_corners, :] -
                                           points[i, :],
    ...
    col = _get_random_color(int(np.mean(img)))
...
def draw_multiple_polygons(img, max_sides=8, nb_polygons=30, **extra):
    ...
    for i in range(nb_polygons):
        ...
        corner_angles = [_angle_between_vectors(new_points[(i-1) % num_corners, :] -
                                               new_points[i, :],
        ...
        if _intersect(segments[:, 0:2, None],
                     segments[:, 2:4, None],
                     new_segments[:, 0:2, :],
                     new_segments[:, 2:4, :],
...
def draw_checkerboard(img, max_rows=7, max_cols=7, transform_params=(0.05, 0.15)):
    ...
    points = _keep_points_inside(warped_points, img.shape[:2])
    return points
...
def draw_cube(img, min_size_ratio=0.2, min_angle_rot=math.pi / 10,
    ...
    col_face = _get_random_color(background_color)
    for i in [0, 1, 2]:
    ...
    # Keep only the points inside the image
    points = _keep_points_inside(points, img.shape[:2])
    return points
...
def visualize_interest_points(img, points):
